...
def load_test_dataset(data_dir, batch_size, num_workers, image_size, shuffle=False, use_test_indices=False):
    """
    ...
    # if use_test_indices:
    #     test_indices = torch.load("./models/test_indices.pth")

    #     test_dataset = Subset(
    #         CustomDirectoryLayoutDataset(root=data_dir, transform=test_transforms),
    #         test_indices
    #     )
    # else:
...
def load_trained_model(model_path, num_labels, device, image_size):
    ...
    model.classifier[2] = nn.Linear(num_features, len(LABEL_ORDER))
...
def evaluate_model(model, test_loader, device, threshold=0.5):
    ...
    metrics = {"loss": test_loss, "exact_match": exact_match, "hamming_acc": hamming_acc,
        "mean_iou": mean_iou, "precision_micro": precision_micro, "recall_micro": recall_micro,"f1_micro": f1_micro,"f1_macro": f1_macro}
    return metrics
